...
const monitor = buildMonitor(
  { redis },
  { name: 'lula.integration', level: 'debug' },
)

describe.skip('lula-hub', () => {
  const state = {
  ...
  beforeAll(async () => {
    ...
    buildMonitor.debug({ startTimeMs: state.startTimeMs }, 'beforeAll')
  ...
  it.only('should get empty items when reading empty stream', async () => {
    const res = await state.fastify.inject({
      method: 'GET',
      url: '/xread/0-0',
      headers: state.authHeader,
